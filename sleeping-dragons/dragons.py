...
#dicontionary of lairs

easy_lair = {
    "dragon" : Actor("dragon-asleep", pos=(600, 100)),
    "eggs": Actor("one-egg", pos=(400, 110)),
    "egg_count": 1,
    "egg_hidden": False,
    "egg_hide_counter": 0,
    "sleep_length": 10,
...
medium_lair = {
    "dragon" : Actor("dragon-asleep", pos=(600, 285)),
...
hard_lair = {
    "dragon" : Actor("dragon-asleep", pos=(600, 470)),
    "eggs": Actor("three-eggs", pos=(400, 490)),
    "egg_count": 3,
    "egg_hidden": False,
    "egg_hide_counter": 0,
    "sleep_length": 4,
...
def draw_lairs(lairs_to_draw):
    for lair in lairs_to_draw: #this is letting you draw each lair in the list
        lair["dragon"].draw()#calling the key dragon and drawing the value
        if lair["egg_hidden"] is False:
            lair["eggs"].draw() #checking if the egg is visible and if it is, draw it

def draw_counters(eggs_collected, lives): #just drawing stuff on the bottom of the screen
...
#moving hero and running the game
...
def check_for_collisions():
    global lairs, eggs_collected, lives, reset_requires, game_complete
    for lair in lairs:
        if lair["egg_hidden"] is False:
...
#what happens when we hit a dragon?
def handle_dragon_collision():
    global reset_required
    reset_required = True
    animate(hero, pos=HERO_START, on_finished=subtract_life) #says animation complete
...
def subtract_life():
    ...
    if lives == 0:
        game_over = True
    reset_required = False #hero is in starting position so this is false

#animate the lair
def update_lairs():
    global lairs, hero, lives
    for lair in lairs:
        if lair["dragon"].image == "dragon-asleep": #checks if dragon is asleep
            update_sleeping_dragon(lair)
        elif lair["dragon"].image == "dragon-awake":
            update_waking_dragon(lair)
        update_egg(lair)

clock.schedule_interval(update_lairs, 1) #runs every second

#functions for animating the lairs
